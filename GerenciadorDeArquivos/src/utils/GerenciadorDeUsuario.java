...
public class GerenciadorDeUsuario {
	...
	public void trocarSenha(String senhaAntiga, String senhaNova, int id) {
		List<Usuario> usuarios = lerUsuarios();
		boolean existe = false;
		for (Usuario usuario : usuarios) {
			if (usuario.getId() == id) {
				...
				if (usuario.getSenha().equals(senhaAntiga)) {
					...
					String pass = sc.next();
					if (senhaNova.equals(pass)) {
						usuario.setSenha(senhaNova);
						reescreverArquivo(usuarios);
						System.out.println("Senha Alterada com sucesso!");
						break;
					} else {
						System.err.println("Senha de confirmação incorreta!");
					}
				} else {
					System.err.println("Senha atual incorreta, tente novamente.");
					break;
				}
			}
